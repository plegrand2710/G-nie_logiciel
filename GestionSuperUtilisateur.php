...
class GestionSuperUtilisateur extends GestionUtilisateur {
    ...
    public function __construct($calendrier, array $activites = []) {
        ...
        if (!is_array($activites)) {
            throw new InvalidArgumentException("La liste des activités doit être une instance de tableau.");
        }
    ...
    private function getCreneauId(DateTime $heureDebut, DateTime $heureFin, int $idActivite): int {
        $stmt = $this->getPDO()->prepare("SELECT idCreneau FROM Creneau WHERE heure_debut = :heureDebut AND heure_fin = :heureFin");
        $stmt->execute([
        ...
        if (!$creneau) {
            $stmt = $this->getPDO()->prepare("INSERT INTO Creneau (heure_debut, heure_fin, duree) VALUES (:heureDebut, :heureFin, :duree)");
            $stmt->execute([
                ':heureDebut' => $heureDebut->format('H:i:s'),
                ':heureFin' => $heureFin->format('H:i:s'),
