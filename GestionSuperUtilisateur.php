...
class GestionSuperUtilisateur extends GestionUtilisateur {
    ...
    public function creer_activite(string $nom, float $tarif, string $duree): Activite {
        ...
        $gestionCreneaux = new GestionCreneauxActivite($nouvelleActivite);
        $this->associerGestionCreneaux($gestionCreneaux);

        return $nouvelleActivite;
    }

    public function supprimer_activite(Activite $activite) {
        if (!$activite instanceof Activite) {
            throw new InvalidArgumentException("L'activité doit être une instance de Activite.");
        }

        $this->_activites = array_filter($this->_activites, function ($act) use ($activite) {
            return $act !== $activite;
        });
    }
    ...
    private function associerGestionCreneaux(GestionCreneauxActivite $gestionCreneaux) {
        $calendrier = $this->getCalendrier();
        $reflect = new ReflectionClass($calendrier);
        $property = $reflect->getProperty('_gestionCreneaux');
        $property->setAccessible(true);

        $gestionCreneauxActuels = $property->getValue($calendrier);
        $gestionCreneauxActuels[] = $gestionCreneaux;

        $property->setValue($calendrier, $gestionCreneauxActuels);
    }
}