...
class GestionUtilisateur {
    ...
    private function verifierDisponibiliteCreneau($idCreneauxActiviteReserve) {
        error_log($idCreneauxActiviteReserve);
        $stmt = $this->_pdo->prepare("SELECT reserver FROM CreneauxActiviteReserve WHERE idCreneauxActivite = :idCreneauxActivite AND reserver = 1");
        $stmt->execute([':idCreneauxActiviteReserve' => $idCreneauxActiviteReserve]);
        $reserver = $stmt->fetchColumn();
        error_log("reserve = ".($reserver));
        if ($reserver) {
