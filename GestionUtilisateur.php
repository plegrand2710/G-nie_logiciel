...
class GestionUtilisateur {
    ...
    public function reserver($creneau, $activite, $personne) {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO Reservation (idReservation, statut, date_reservation, idPersonne, idCreneau, idActivite)
            VALUES (:id, :statut, NOW(), :idPersonne, :idCreneau, :idActivite)
        ");
        $stmt->execute([
    ...
    public function paiementActivite($personne, $activite) {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO Paiement (montant, date_paiement, idPersonne, idActivite)
            VALUES (:montant, NOW(), :idPersonne, :idActivite)
        ");
        $stmt->execute([
            ':montant' => $montant,
            ':idPersonne' => $personne->getId(),
    ...
    public function remboursementActivite($personne, $activite, $penalite): void {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO Remboursement (montant, penalite, date_remboursement, idPersonne, idActivite)
            VALUES (:montant, :penalite, NOW(), :idPersonne, :idActivite)
        ");
        $stmt->execute([
            ':montant' => $activite->getTarif(),
            ':penalite' => $penalite,
    ...
    public function supprimerReservation($reservation): void {
        ...
        foreach ($this->_reservations as $key => $reservation) {
            if ($reservation->getId() === $reservationId) {
                ...
                $reservation->annulerReservation();
                $stmt = $pdo->prepare("DELETE FROM Reservation WHERE idReservation = :id");
    ...
    public function afficherReservationsParUtilisateur($personne): array {
        ...
        ");
        $stmt->execute([':idPersonne' => $personne->getId()]);

        $reservations = [];
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $reservations[] = new Reservation(
                $row['idReservation'],
                $this->retrouverCreneau($row['idCreneau']),
                $this->retrouverActivite($row['idActivite']),
                $personne
            );
        }
    ...
    public function afficherReservationsUtilisateurParActivite($personne, $activite): array {
        ...
        $stmt = $pdo->prepare("
            SELECT * FROM Reservation 
            WHERE idPersonne = :idPersonne AND idActivite = :idActivite
        ");
        $stmt->execute([
            ':idPersonne' => $personne->getId(),
            ':idActivite' => $activite->getId(),
